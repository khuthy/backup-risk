...
function display_risk_source_view($source)
{
    ...
    echo "</div>\n";
    echo "<div class=\"span7\">\n";
    echo "<input style=\"cursor: default;\" type=\"text\" name=\"source\" id=\"source\" size=\"50\" value=\"" . $escaper->escapeHtml(get_names_by_multi_values("source", $source)) . "\" title=\"" . $escaper->escapeHtml(get_name_by_value("source", $source)) . "\" disabled=\"disabled\" />\n";
    echo "</div>\n";
    echo "</div>\n";
}
...
function display_risk_source_edit($source)
{
    ...
    create_multiple_dropdown("source", $source, NULL, NULL, false, "", "", true, " class='multiselect' ");
    echo "</div>\n";
    echo "</div>\n";
 ...
 * FUNCTION: DISPLAY MITIGATION TEAM VIEW *
 ******************************************/
function display_control_effectiveness_view($control_effectiveness)
{
    global $lang, $escaper;
    ...
    echo "</div>\n";
    echo "<div class=\"span7\">\n";
    echo "<span>".$escaper->escapeHtml( $control_effectiveness)."</span>\n";
    echo "</div>\n";
    echo "</div>\n";
}
/********************************************
...
*********************************************/
function display_mitigation_percent_view($mitigation_controls)
{
    global $lang, $escaper;
    ...
    echo "</div>\n";
    echo "<div class=\"span7\">\n";
    echo "<input style=\"cursor: default;\" type=\"text\" name=\"mitigation_percent\" id=\"mitigation_percent\" size=\"50\" value=\"" . $escaper->escapeHtml($mitigation_controls) . "\" disabled=\"disabled\" />\n";
    echo "</div>\n";
    echo "</div>\n";/**/
}
...
/***********************************************
 * FUNCTION: DISPLAY RISK EFFECTIVENESS *
 ***********************************************/
function display_risk_effectiveness_view($security_requirements)
{
    global $lang, $escaper;
    
    echo "<div class=\"row-fluid\">\n";
    echo "<div class=\"span5 text-right\" >\n";
    echo $escaper->escapeHtml($lang['RiskEffectiveness']) .":  \n";
    echo "</div>\n";
    echo "<div class=\"span7\">\n";
    echo "<textarea style=\"cursor: default;\" name=\"security_requirements\" cols=\"50\" rows=\"3\" id=\"security_requirements\" title=\"" . $escaper->escapeHtml($security_requirements) . "\" disabled=\"disabled\">" . $escaper->escapeHtml($security_requirements) . "</textarea>\n";
    echo "</div>\n";
...
/*****************************************
* FUNCTION: DISPLAY CONTROL EFFECTIVENESS*
******************************************/
function display_control_effectiveness_edit($control_effectiveness)
{
    global $lang, $escaper;
    ...
    echo "
        <div class=\"row-fluid\">
            ...
            echo "<div class=\"span7\">";
            $mitigationimpacts = array(' '=>'--','Very Good'=>'Very Good - 100%','Good'=>'Good - 80%','Satisfactory'=>'Satisfactory - 50%','Weak'=>'Weak - 30%','UnSatisfactory'=>'UnSatisfactory - 10%');
            
            echo '<select name="control_effectiveness" id="control_effectiveness">';
            ...
            foreach ($mitigationimpacts as $selection) {
                ...
                echo '<option '.$GLOBALS['control_effectiveness'].' value="'.$selection.'">'.$selection.'</option>';
            }
            
            echo '</select>'; 
           // $eID = "control_effectiveness";
        ...
        <script>
            $(document).ready(function(){
                $('#".$eID."').select({
                    ...
                    maxHeight: 250,
                    dropUp: true,
                    filterPlaceholder: '".$escaper->escapeHtml($lang["SelectForControlsEffectiveness"])."',
                    onDropdownHide: function(){
                        var form = $('#{$eID}').parents('form');
                        var tableId = $(\".controls-effectiveness-table-container\", form).data('tableid');
...
function display_controls_effectiveness_edit($mitigation_controls)
{
    global $lang, $escaper;
    ...
    echo "
        ...
        echo "<div class=\"span7\">";
        $mitigationimpacts = array('Insignificant'=>'', 'Minor'=>'','Moderate'=>'','Major'=>'','Extreme'=>'');
        
        echo '<select name="mitigationimpact">
                        <option value="1">Mitigation Impact</option>';
        $options = isset($post['selectedimpact']) ? $post['selectedimpact'] : 0;
        foreach ($mitigationimpacts as $selection) {
...
/***********************************************
 * FUNCTION: DISPLAY RISK EFFECTIVENESS  EDIT *
 ************************************************/
function display_risk_effectiveness_edit($security_requirements)
{
    global $lang, $escaper;
    ...
    echo "
        <div class=\"row-fluid\">
            <div class=\"span5 text-right\" id=\"SecurityRequirementsTitle\">"
        .$escaper->escapeHtml($lang['RiskEffectiveness']) .":
            </div>
            <div class=\"span7\">
                <textarea class=\"active-textfield\" title=\"".$escaper->escapeHtml($lang['SecurityRequirements']) ."\" name=\"security_requirements\" cols=\"50\" rows=\"3\" id=\"security_requirements\" tabindex=\"1\">" . $escaper->escapeHtml($security_requirements) . "</textarea>
            </div>
