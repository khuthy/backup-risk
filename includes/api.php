 ...
 * This function is called through the API * linderep
 *******************************************/
...
function getTabHtml($id, $template){
    ...
    if ($mitigation == true && $access)
    {
        ...
        $controls_effectiveness = $mitigation[0]['control_effectiveness'];
    }
    // Otherwise
    else
    {
...
function saveMitigationForm()
{
    ...
    //    $mitigation_percent = (int)$_POST['mitigation_percent']; 
        $mitigation_percent =  0;
        switch ($_POST['control_effectiveness']) {
            case 'Very Good - 100%':
...
function saveMitigation(){
    ...
    elseif(isset($_SESSION["modify_risks"]) && $_SESSION["modify_risks"] == 1 && $access){
        ...
        //    'mitigation_percent'        => get_param("POST", "mitigation_percent"),
            'mitigationlikelihood'      => get_param("POST", "mitigationlikelihood"),
            'mitigationimpact'          => get_param("POST", "mitigationimpact")
        );
...
function getMitigationControlsDatatable(){
    ...
    $control_effectiveness = $_GET['control_effectiveness'];
    ...
    foreach ($controls as $key=>$control)
    {   
        switch ($control['mitigation_percent']) {
            case 100:
                $control_effectiveness = 'Very Good - 100%';
                break;
            case 80:
                $control_effectiveness = 'Good - 80%';
                break;
            case 50:
                $control_effectiveness = 'Satisfactory - 50%';
                break;
            case 30:
                $control_effectiveness = 'Weak - 30%';
                break;
            case 10:
                $control_effectiveness = 'UnSatisfactory - 10%';
                break;
        ...
        $delete = "";
        $clone  = "";
        $edit = '<a href="#" class="control-block--edit" title="'.$escaper->escapeHtml($lang["Edit"]).'" data-id="'.$escaper->escapeHtml($control['id']).'"><i class="fa fa-pencil-square-o"></i></a>';
        $html = "<div class='control-block item-block clearfix'>\n";
            $html .= "<div class='control-block--header clearfix' data-project=''>\n";
            $html .= "<div class='checkbox-in-div'>\n";
            $html .= "<input type='checkbox' name='control_ids[]' value='".$escaper->escapeHtml($control['id'])."'>\n";
                ...
                $html .= "<div class='control-block--row'>\n";
                    $html .= "<table width='100%'>\n";
                        ...
                        $html .= "<tr>\n";
                            ...
                            $html .= "<td>".$escaper->escapeHtml($control['family_short_name'])."</td>\n";
                            $html .= "<td width='200px' align='right'><strong>".$escaper->escapeHtml($lang['MitigationPercent'])."</strong>: </td>\n";
                            $html .= "<td>".$escaper->escapeHtml($control_effectiveness/*$control['mitigation_percent']*/)." </td>\n";
                        $html .= "</tr>\n";
                        $html .= "<tr>\n";
                            $html .= "<td align='right'><strong>".$escaper->escapeHtml($lang['Description'])."</strong>: </td>\n";
...
/****************************
 * FUNCTION: UPDATE CONTROL ON THE RISK *
 ****************************/
function updateControl()
{
    ...
    /*echo'<br/>=============Update Framework Control=============<br/>';
     print"<pre/>";
     print_r($_POST);
     echo'<br/>=============Update Framework Control=============<br/>';
      exit();*/
    ...
    elseif (is_int($control_id))
    {
        ...
        $mitigation_percent = $_POST['mitigation_percent'];
        $risk_id            = $_POST['risk_id']; 
        if($risk_id > 0){
        $id = (int)$risk_id - 1000;
        switch ($mitigation_percent) {
            case 100:
                $control_effectiveness = 'Very Good - 100%';
                break;
            case 80:
        ...
        //Update the Control Effectiveness 
        // Update the risk
        $db   = db_open();
        $stmt = $db->prepare("UPDATE mitigations SET  control_effectiveness=:control_effectiveness WHERE risk_id=:risk_id");
        ...
        // Display an alert
        set_alert(true, "good", "An existing control was updated successfully.Yes Yes");
