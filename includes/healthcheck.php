...
function simplerisk_health_check()
{
	...
	//linde $latest_app_version = latest_version("app");
	$current_db_version = current_version("db");
	//linde $latest_db_version = latest_version("db");

	//linde echo "<b><u>SimpleRisk Versions</u></b><br />";

	// Check that we are running the latest version of the SimpleRisk application
	//linde check_app_version($current_app_version, $latest_app_version);

	// Check that we are running the latest version of the SimpleRisk database
	//linde check_db_version($current_db_version, $latest_db_version);

	// Check that the application and database versions are the same
	//linde check_same_app_and_db($current_app_version, $current_db_version);
...
function check_same_app_and_db($current_app_version, $current_db_version)
{
	...
	{
		health_check_good("The Geoscience application and database are at the same versions (" . $current_db_version . ").");
	}
	else
	{
		health_check_bad("The Geoscience application (" . $current_app_version . ") and database (" . $current_db_version . ") versions are not the same.");
	}
}
...
function check_simplerisk_directory_permissions()
{
	...
	{
		health_check_good("The Geoscience directory (" . $simplerisk_dir . ") is writeable by the web user.");
	}
	else
	{
		health_check_bad("The Geoscience directory (" . $simplerisk_dir . ") is not writeable by the web user.");
	}
...
{
	// URLs to check
	//linde $urls = array("https://register.simplerisk.com", "https://services.simplerisk.com", "https://updates.simplerisk.com", "https://vfeed.simplerisk.com");
	...
	foreach ($urls as $url)
	{
		...
		{
			health_check_good("Geoscience connected to " . $url . ".");
		}
		else
		{
			health_check_bad("Geoscience was unable to connect to " . $url . ".");
		}
	}
}
...
function check_database_connectivity()
{
	...
	if ($db !== null)
	{
		...
		health_check_good("Communicated with the Geoscience database successfully.");
	}
	else
	{
		health_check_bad("Unable to communicate with the geoscience database.");
	}
}
...
function check_api_connectivity()
{
	...
	{
		health_check_good("Communicated with the Geoscience API successfully.");
	}
	else
	{
		health_check_bad("Unable to communicate with the Geoscience API.");
	}
}
...
function check_strict_sql_mode()
{
	...
	{
		health_check_bad("Geoscience will not work properly with STRICT_TRANS_TABLES enabled.");
	}
	else
	{
...
function check_no_zero_date()
{
        ...
        {       
                health_check_bad("Geoscience will not work properly with NO_ZERO_DATE enabled.");
        }
        else    
        {       
...
function check_mysql_size()
{
	...
	health_check_good("Geoscience is using " . round($size, 2) . " MB of disk space.");
}
...
function check_only_full_group_by()
{
        ...
        {       
                health_check_bad("Geoscience will not work properly with ONLY_FULL_GROUP_BY enabled.");
        }
        else    
        {       
...
function check_php_version()
{
	...
	{
		health_check_good("Geoscience is running under PHP 7.");
	}
	// If this is PHP 5.x
	else if (PHP_VERSION_ID >= 50000 && PHP_VERSION_ID < 60000)
	{
		health_check_bad("Geoscience will no longer run properly under PHP version 5.x.  Please upgrade to PHP 7.");
	}
	else
	{
		health_check_bad("Geoscience requires PHP 7 to run properly.");
	}
}
