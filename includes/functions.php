 ...
 *************************/
//linde function get_setting($setting, $default=false)
 ...
 *************************/
function submit_risk($status, $subject, $reference_id, $regulation, $control_number, $location, $source,  $category, $team, $technology=0, $owner, $manager, $assessment, $notes, $project_id = 0, $submitted_by=0, $submission_date=false, $additional_stakeholders="")
{
    $submitted_by || ($submitted_by = $_SESSION['uid']);

    $owner || $owner = 0;
    echo "tying to submits";
    // Open the database connection
    $db = db_open();

    // Set numeric null to 0
    if ($location == NULL) $location = "";
    //if ($technology == NULL) $technology = Null;
...
function submit_mitigation($risk_id, $status, $post, $submitted_by_id=false)
{
   ...
   // $mitigation_percent         = (isset($post['mitigation_percent']) && $post['mitigation_percent'] >= 0 && $post['mitigation_percent'] <= 100) ? $post['mitigation_percent'] : 0;
    ...
    $mitigation_controls        = is_array($mitigation_controls) ? implode(",", $mitigation_controls) : $mitigation_controls;
        ...
        //linlast
   // $mitigationimpact  =  (isset($post['mitigationimpact']) && $post['mitigationimpact'] > 0 && $post['mitigationimpact'] <= 5) ? $post['mitigationimpact'] : 1;
   $mitigationimpact = isset($post['mitigationimpact'])  ? $post['mitigationimpact'] : "" ;
   $mitigationlikelihood = isset($post['mitigationlikelihood'])  ? $post['mitigationlikelihood'] : ""; ;

   //$mitigationlikelihood  = (isset($post['mitigationlikelihood']) && $post['mitigationlikelihood'] > 0 && $post['mitigationlikelihood'] <= 5) ? $post['mitigationlikelihood'] : 1;
	//$mitigation_percent = (isset($post['mitigation_percent']) && $post['mitigation_percent'] >= 0 && $post['mitigation_percent'] <= 100) ? $post['mitigation_percent'] : 0;

   // array('remote', 'Unlikely','Credible','Likely','Almost Certain');

    $mitigationimpactvalue  = Null;
    $mitigationlikelihoodvalue = Null;
    
    switch ($mitigationlikelihood) {
        case 'remote':
              $mitigationlikelihoodvalue = 1;
          break;
        case 'Unlikely':
             $mitigationlikelihoodvalue = 2;
          break;
        case 'Credible':
            $mitigationlikelihoodvalue = 3;
          break;
        case 'Likely':
            $mitigationlikelihoodvalue = 4;
          break;
        case 'Almost Certain':
           $mitigationlikelihoodvalue = 5;
         break;
         default:
         $mitigationlikelihoodvalue = 0;

      }

    switch ($mitigationimpact) {
        case 'Insignificant':
              $mitigationimpactvalue = 1;
          break;
        case 'Minor':
             $mitigationimpactvalue = 2;
          break;
        case 'Moderate':
            $mitigationimpactvalue = 3;
          break;
        case 'Major':
            $mitigationimpactvalue = 4;
          break;
        case 'Extreme':
            $mitigationimpactvalue = 5;
         break;
         default:
         $mitigationimpactvalue = 1;
      }

    $score25 = ($mitigationimpactvalue * $mitigationlikelihoodvalue);
    $score26 = $score25 * 4;
    
   // echo $score25;
    $mitigation_percent = $score26;
    $mitigation_percent =  0;
    switch ($post['control_effectiveness']) {
    	case 'Very Good - 100%':
    	...
    	case 'Good - 80%':
    		...
    		break;
    	case 'Satisfactory - 50%':
    		$mitigation_percent = 50;
    		break;
    	case 'Weak - 30%':
    		$mitigation_percent = 30;
    		break;
    	case 'UnSatisfactory - 10%':
    ...
    // Open the database connection
    $db = db_open(); 
    $control_effectiveness = isset($post['control_effectiveness']) ? $post['control_effectiveness'] : ""; 
    ...
    $stmt = $db->prepare("INSERT INTO mitigations (`risk_id`, `planning_strategy`, `mitigation_effort`, `mitigation_cost`, `mitigation_owner`, 
    		...
    		`submission_date`, `mitigation_percent`, `mitigation_controls`, `control_effectiveness`,`residual_iikelihood`, `residual_impact`) 
    		VALUES (:risk_id, :planning_strategy, :mitigation_effort, :mitigation_cost, :mitigation_owner, :mitigation_team, 
    		:current_solution, :security_requirements, :security_recommendations, :submitted_by, :planning_date, :submission_date, 
    		:mitigation_percent, :mitigation_controls, :control_effectiveness,:residual_iikelihood,:residual_impact)");
    $stmt->bindParam(":risk_id", $id, PDO::PARAM_INT);
    ...
    $stmt->bindParam(":control_effectiveness", $control_effectiveness, PDO::PARAM_STR);
    $stmt->bindParam(":residual_iikelihood", $mitigationlikelihood, PDO::PARAM_STR);
    $stmt->bindParam(":residual_impact", $mitigationimpact, PDO::PARAM_STR);
    $stmt->execute();   
  ...
  //$message = "A mitigation was submitted for risk ID \"" . $risk_id . "\" by username \"" . $_SESSION['user'] . "\".";
    $message = "A mitigation was submitted for risk ID \"" . $risk_id . "\".";
    write_log($risk_id,1, $message);
...
function get_residual_risk($risk_id)
{
    ...
    // Query the database
    $stmt = $db->prepare("
        SELECT t2.calculated_risk, GREATEST(IFNULL(t3.mitigation_percent, 0), IFNULL(0, 0)) AS mitigation_percent, 
    		 t3.control_effectiveness,t3.residual_iikelihood,t3.residual_impact
        FROM risks t1
    ...
    if($risk['mitigation_percent'] ==0){
    	 
    switch ($risk['control_effectiveness']) {
    	case 'Very Good':
    		$risk['mitigation_percent'] = 100;
    		break;
    	case 'Very Good - 100%':
    		$risk['mitigation_percent'] = 100;
    		break;
    	case 'Good':
    		$risk['mitigation_percent'] = 80;
    		break;
    	case 'Good - 80%':
    		$risk['mitigation_percent'] = 80;
    		break;
    	case 'Satisfactory':
    		$risk['mitigation_percent'] = 50;
    		break;
    	case 'Satisfactory - 50%':
    		$risk['mitigation_percent'] = 50;
    		break;
    	case 'Weak':
    		$risk['mitigation_percent'] = 30;
    		break;
    	case 'Weak - 30%':
    		$risk['mitigation_percent'] = 30;
    		break;
    	case 'UnSatisfactory':
    		$risk['mitigation_percent'] = 10;
    		break;
    	case 'UnSatisfactory - 10%':
    		$risk['mitigation_percent'] = 10;
    		break;
    	default:
    		$risk['mitigation_percent'] = 0;
    } 
    }
    $residual_impact  = 0;
    switch ($risk['residual_impact']) {
    ...
    $residual_iikelihood = 0;
    switch ($risk['residual_iikelihood']) {
    	// $mitigationlikelihood = array('1', '2','3','4','5');
    	case'Rare':
    	    $residual_iikelihood = 1;
    	    break;
    	case'Unlikely':
	    	$residual_iikelihood = 2;
	    	break;
    	case'Moderate':
	    	$residual_iikelihood = 3;
	    	break;
    	case'Likely':
	    	$residual_iikelihood = 4;
	    	break;
    	case 'Almost Certain':
	    	$residual_iikelihood = 5;
	    	break;
    }
     
    $residual_risk = round($residual_iikelihood * $residual_impact);
    
    //$residual_risk = round($risk['mitigation_percent'] / 4, 0);
//echo'<br/>Residual Risk: '.$residual_risk;
...
function update_mitigation($risk_id, $post)
{
    ...
    //linlast real updatemitigation variables
   ...
   $mitigationimpact = isset($post['mitigationimpact']) ? $post['mitigationimpact'] : ""; 
   $mitigationlikelihood = isset($post['mitigationlikelihood']) ? $post['mitigationlikelihood'] : ""; 
   $control_effectiveness = $post['control_effectiveness'];
   //$mitigationlikelihood  = (isset($post['mitigationlikelihood']) && $post['mitigationlikelihood'] > 0 && $post['mitigationlikelihood'] <= 5) ? $post['mitigationlikelihood'] : 1;
	//$mitigation_percent = (isset($post['mitigation_percent']) && $post['mitigation_percent'] >= 0 && $post['mitigation_percent'] <= 100) ? $post['mitigation_percent'] : 0;
...
switch ($control_effectiveness) {
   ...
   // echo $score25;
    ...
    $message = "Risk mitigation details were updated for risk ID \"" . $risk_id."\".";
    write_log($risk_id, $_SESSION['uid'], $message);
...
function registration_redirect()
{
    ...
    if (get_setting('registration_registered') == 0)
    {
        ...
        if (isset($_SESSION["admin"]) && $_SESSION["admin"] == "1")
        {
            ...
            {
                // Set the alert
                set_alert(true, "good", "Welcome, You Are now Logged into the system");

                // Redirect to the register page
                //linde header("Location: admin/register.php");
